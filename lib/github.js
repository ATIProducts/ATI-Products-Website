...
async function getFile(filePath, ref) {
  ...
  try {
    const data = await ghFetch(
      `/repos/${owner}/${repo}/contents/${encodePath(filePath)}?ref=${encodeURIComponent(ref || branch)}`
    );
...
// The commit the site's branch currently points at.
async function getHeadSha() {
  const { owner, repo, branch } = repoConfig();
  const data = await ghFetch(`/repos/${owner}/${repo}/git/ref/heads/${encodePath(branch)}`);
  return data.object.sha;
}

// Saves several text files as ONE commit (so the site redeploys once, and
// the history shows one change). `parentSha` must be the commit the files
// were read from: if anyone else saved something in the meantime, GitHub
// refuses the update instead of silently overwriting their work.
async function commitFiles(files, message, parentSha) {
  const { owner, repo, branch } = repoConfig();
  const base = `/repos/${owner}/${repo}/git`;
  const parent = await ghFetch(`${base}/commits/${parentSha}`);
  const tree = await ghFetch(`${base}/trees`, {
    method: 'POST',
  ...
  const actor = currentActor();
  const body = { message, tree: tree.sha, parents: [parentSha] };
  if (actor) {
    body.message = `${message} (by ${actor.name})`;
    body.author = { name: actor.name, email: actor.email || `${actor.username}@ati-website-admin.local` };
  }
  const post = (b) => ghFetch(`${base}/commits`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(b) });
  let made;
  try {
    made = await post(body);
  } catch (e) {
    if (e.status !== 422 || !body.author) throw e;
    const retry = Object.assign({}, body);
    delete retry.author;
    made = await post(retry);
  }
  try {
    await ghFetch(`${base}/refs/heads/${encodePath(branch)}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
    ...
    if (e.status === 422 || e.status === 409) {
      const err = new Error('Someone else saved a change to the website at the same moment. Nothing was changed — please try again.');
      err.status = 409;
      err.expose = true;
      throw err;
    }
...
module.exports = { getFile, putFile, putBinaryFile, listDir, listCommits, getHeadSha, commitFiles, repoConfig };
