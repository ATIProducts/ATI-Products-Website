...
// `profile` holds the main admin's own settings saved from the admin
// screen (currently just their display name). It lives inside the same
// encrypted file as everyone else, so it needs no extra Vercel setting.
function mainAdmin(profile) {
  ...
  const name =
    ...
    'Main admin';
  return { username, name, role: 'admin', ver, builtIn: true, email: '' };
}

// The main admin with their saved name filled in. If the user list can't
// be read for any reason, the main admin still works (under the default
// name) — it's the way back in when something goes wrong.
async function loadMainAdmin() {
  try {
    const { main } = await loadUsers();
    return mainAdmin(main);
  } catch {
    return mainAdmin();
  }
}
...
function encryptList(users, main) {
  const iv = crypto.randomBytes(12);
  const cipher = crypto.createCipheriv('aes-256-gcm', fileKey(), iv);
  const data = Buffer.concat([cipher.update(JSON.stringify({ users, main: main || {} }), 'utf8'), cipher.final()]);
  return (
    JSON.stringify(
      {
...
let cache = null; // { users, main, sha, at }
...
async function loadUsers(opts) {
  ...
  if (!file) {
    cache = { users: [], main: {}, sha: null, at: Date.now() };
    return cache;
  }
  const unlocked = decryptList(file.content);
  cache = { users: unlocked.users, main: unlocked.main, sha: file.sha, at: Date.now() };
  return cache;
}

async function saveUsers(users, sha, message, main) {
  main = main || {};
  const result = await putFile(USERS_PATH, encryptList(users, main), message, sha || undefined);
  cache = {
    users,
    main,
...
async function updateUsers(mutator, message) {
  ...
  const main = Object.assign({}, current.main);
  const result = mutator(users, main);
  await saveUsers(users, current.sha, message, main);
  return result;
}
...
async function resolveSession(session) {
  ...
  if (session.u === main.username) {
    return session.v === main.ver ? loadMainAdmin() : null;
  }
